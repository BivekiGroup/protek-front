...
const FEATURES = [
  {
    ...
    subtitle: 'Сохраняйте свои машины для быстрого подбора',
  },
  {
    title: 'Сохраняйте избранное',
    subtitle: 'Не теряйте важные товары и подборки',
  },
  {
    title: 'Отслеживайте заказы',
    subtitle: 'Статусы, история и повторные покупки',
  },
];

const LoginRequiredPrompt: React.FC = () => {
  const router = useRouter();

  const openAuth = React.useCallback(() => {
    if (!router.isReady) return;
    const nextQuery = { ...router.query, openAuth: '1' };
        ...
        <div className="bg-white/80 backdrop-blur border border-gray-100 shadow-xl rounded-2xl p-6 md:p-10">
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-3">
            {FEATURES.map(({ title, subtitle }) => (
              <div key={title} className="flex items-center gap-3 rounded-xl border border-gray-100 bg-gray-50 p-4">
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3 sm:items-center">
            <button
              onClick={openAuth}
              className="inline-flex justify-center items-center gap-2 rounded-xl bg-[#EC1C24] px-5 py-3 font-semibold text-white hover:text-white shadow-sm hover:bg-[#DC1C24] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#EC1C24]/30 transition"
            >
